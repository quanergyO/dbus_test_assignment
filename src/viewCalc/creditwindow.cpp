...
CreditWindow::CreditWindow(std::unique_ptr<DbusFramework> &dbus,
                           QWidget *parent)
...
void CreditWindow::on_pushButton_calc_credit_clicked() {
  auto reply = dbus_->callMethod(
      "CreditCalc", "calculate",
      QStringList{ui->doubleSpinBox_deposit_amount->text(),
                  ui->spinBox_term->text(), ui->spinBox_insert_rate->text(),
